...
fun ActivityScreen(viewModel: ActivityViewModel = koinViewModel()) {
    ...
    LazyColumn(
        ...
        item {
            ...
            Text("Activity", style = MaterialTheme.typography.headlineMedium, color = OuraColors.OnSurface)
        }
        ...
        if (state.workouts.isNotEmpty()) {
            item {
                Text("Workouts", style = MaterialTheme.typography.titleMedium, color = OuraColors.OnSurface)
            }
            state.workouts.take(10).forEach { w ->
                item {
                    FlowRow(
